...
export async function GET(req: Request) {
  ...
  const items = await prisma.employee.findMany({ where, orderBy: { name: "asc" } });
...
export async function POST(req: Request) {
  ...
  const { name, phone, position, baseSalary } = body;
  if (!name) return NextResponse.json({ error: "name is required" }, { status: 400 });
  const locationId = session.user.role === "super_admin" ? body.locationId : session.user.locationId!;
  const item = await prisma.employee.create({ data: { name, phone, position, baseSalary, locationId } });
